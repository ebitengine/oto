...
func (i *_IAudioClient2) GetMixFormat() (*_WAVEFORMATEX, error) {
	var deviceFormat *_WAVEFORMATEX
	r, _, _ := syscall.Syscall(i.vtbl.GetMixFormat, 2, uintptr(unsafe.Pointer(i)), uintptr(unsafe.Pointer(&deviceFormat)), 0)
	if uint32(r) != uint32(windows.S_OK) {
		return nil, fmt.Errorf("oto: IAudioClient2::GetMixFormat failed: HRESULT(%d)", uint32(r))
	}
	return deviceFormat, nil
}
...
func (i *_IAudioClient2) Initialize(shareMode _AUDCLNT_SHAREMODE, streamFlags uint32, hnsBufferDuration _REFERENCE_TIME, hnsPeriodicity _REFERENCE_TIME, pFormat *_WAVEFORMATEX, audioSessionGuid *windows.GUID) error {
	var r uintptr
